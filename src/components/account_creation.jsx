...
export default function BuyerAccountCreation() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const token = localStorage.getItem("token");
      const response = await fetch("https://movie-nova-5.onrender.com/profile/setup", {
        method: "POST",
        headers: {
      ...
      console.error(err);
      toast("Error submitting profile: " + err.message, "error");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="min-h-screen w-full bg-white flex justify-center py-10">
      <div className="w-full max-w-md min-h-screen bg-white rounded-3xl p-6">
        ...
        {/* PHOTO */}
        {step === "photo" && (
          <FormSection title="Profile Picture" onBack={() => setStep("basic")} onNext={handleSubmit} isLast submitting={submitting}>
...
function FormSection({ title, children, onBack, onNext, isLast = false, submitting = false }) {
  return (
    <div className="space-y-5">
      ...
      <div className="flex gap-2 pt-2">
        ...
        <button
          ...
          className={`flex-1 py-3 rounded-xl text-white flex items-center justify-center gap-2 ${isLast ? "bg-green-600" : "bg-blue-600 hover:bg-blue-700"} disabled:opacity-60`}
        >
          {isLast && submitting ? (
