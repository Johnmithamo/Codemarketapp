...
const ChatProvider = ({ children }) => {
  ...
  const API_URL = "https://movie-nova-5.onrender.com";
...
const SearchBar = ({ value, onChange }) => (
  <div className="px-4 pb-3" data-tour-id="messages-search">
...
useEffect(() => {
  fetch(`https://movie-nova-5.onrender.com/chats/${chat._id}/messages`, {
    headers: {
      Authorization: `Bearer ${token}`,
    },
  })
...
/* ================= APP ================= */
export default function App({ initialChat, onInitialChatConsumed }) {
  const [page, setPage] = useState(initialChat ? "chat" : "messages");
  const [selectedChat, setSelectedChat] = useState(initialChat || null);
  useTourRegister("messages", setPage);

  // If we're handed a chat to open (e.g. buyer tapped "Chat" on a seller's
  // profile), jump straight into it instead of the list.
  useEffect(() => {
    if (initialChat) {
