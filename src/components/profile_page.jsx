...
export default function ProfilePage() {
  ...
  useTourRegister("profileSeller", setPage);
  ...
  return (
    <div className="relative h-full">
      ...
      <div className="absolute top-[110px] left-1/2 -translate-x-1/2 w-[90%] bg-white rounded-2xl p-5 text-center shadow-md py-10">
        ...
        <button data-tour-id="profile-edit" className="mt-5 w-full bg-gray-100 text-blue-600 text-sm py-2 rounded-lg font-medium">
          Edit Profile
        </button>
      </div>
      ...
      <div className="mt-[150px] px-4 space-y-3 py-16">
        <MenuItem data-tour-id="profile-my-services" title="My Services" icon={<User size={18} />} onClick={() => go("services")} />
        <MenuItem data-tour-id="profile-saved" title="Saved Items" icon={<Bookmark size={18} />} onClick={() => go("saved")} />
        <MenuItem data-tour-id="profile-activity" title="Activity" icon={<Activity size={18} />} onClick={() => go("activity")} />
        <MenuItem data-tour-id="profile-settings" title="Settings" icon={<Settings size={18} />} onClick={() => go("settings")} />
      </div>
    </div>
  );
}
...
function Services({ go }) {
  ...
  const addService = async () => {
    if (!newService.title || !newService.price || !newService.image || !newService.category) {
      toast("Please provide title, price, category, and image.", "error");
      return;
    }
...
const updateService = async (index) => {
  ...
  if (!service.title || !service.price || !service.category) {
    toast("Please provide title, price, and category.", "error");
    return;
  }
...
function MenuItem({ icon, title, onClick, ...rest }) {
  return (
    <div onClick={onClick} {...rest} className="flex justify-between items-center bg-white px-4 py-3 rounded-xl cursor-pointer">
